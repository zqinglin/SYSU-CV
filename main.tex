...
% 自定义颜色
\definecolor{primary_color}{RGB}{0, 94, 52}    % 中大绿
\definecolor{secondary_color}{RGB}{0, 0, 0} % 黑
...
% 学院
\newcommand{\school}{计算机学院 | School of Computer Science and Engineering} 
...
\newcommand{\contact}{
    ...
    \textcolor{white}{
        % 邮箱
        \faEnvelope \quad \href{mailto:youremail@seu.edu.cn}{youremail@mail2.sysu.edu.com}
        \hspace{4em}
        % 手机号
...
%%%%%%%%%%%%%%%%%%%%
    ...
    % 页眉：中大绿背景矩形 + 学院名（不包含校徽图片，如果需要校徽，你需要提供其LaTeX路径）
    \begin{tikzpicture}[remember picture, overlay]
        % 绘制页眉的矩形背景
        \fill[fill=primary_color] (current page.north west) rectangle ([yshift=-3.5em]current page.north east);
        
        % 显示学院名称 (这里假设你已经有 sysu_horizontal_white.png 图片在 images/ 目录下)
        % 如果没有图片，请注释掉校徽相关的行，只保留文字。
        \node[anchor=west] at ([xshift=0.4cm, yshift=-1.67em]current page.north west){
            \includegraphics[height=2.8em]{images/sysu_horizontal_white.png} % 假设的中大校徽（白色横版）
        };
        
        \node[anchor=east] at ([xshift=-0.2cm, yshift=-1.8em]current page.north east){
    ...
    \begin{tikzpicture}[remember picture, overlay]
        % 绘制页脚的矩形背景
        \fill[fill=primary_color] (current page.south west) rectangle ([yshift=2.5em]current page.south east);
        
        % 联系方式
        \node[anchor=center] at ([yshift=1.25em]current page.south){\contact};
    \end{tikzpicture}

    % 背景 
    \begin{tikzpicture}[remember picture, overlay]
        \node[opacity=0.04] at(current page.center){
            \includegraphics[width=0.6\paperwidth, keepaspectratio]{images/sysu_logo.png}
        };
    \end{tikzpicture}
    ...
    \begin{minipage}[t]{0.78\textwidth}
        ...
        % {\large \textbf{中山大学}}，专科 \hfill 2000年9月--2010年6月
        % \begin{itemize}
        %     \item 你的学院，你的专业
